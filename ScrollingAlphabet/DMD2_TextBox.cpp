...
DMD_TextBox::DMD_TextBox(DMDFrame &dmd, int left, int top, int width, int height,bool inv) :
  dmd(dmd),
  inverted(inv),
  left(left),
  top(top),
...
size_t DMD_TextBox::write(uint8_t character) {
  ...
  while((cur_x > 0 && cur_x + char_width >= this->width)) {
  ...
  dmd.drawChar(cur_x+left,cur_y+top,character, inverted ? GRAPHICS_OFF : GRAPHICS_INVERSE);
  cur_x += char_width;
  return 1;
}
...
//   //   dmd.drawFilledBox(cur_x+left,cur_y+top,left+width,cur_y+top+rowHeight, inverted ? GRAPHICS_ON : GRAPHICS_INVERSE);
//   // }
...
void DMD_TextBox::scrollY(int scrollBy) {
  if(abs(scrollBy) >= height) { // scrolling over the whole display
    ...
    dmd.drawFilledBox(left, top, left+width-1, top+height-1, inverted ? GRAPHICS_ON : GRAPHICS_INVERSE);
...
void DMD_TextBox::clear() {
  this->reset();
  dmd.drawFilledBox(left,top,left+width,top+height,inverted ? GRAPHICS_ON : GRAPHICS_INVERSE);
