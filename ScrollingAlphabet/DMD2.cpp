...
SoftDMD::SoftDMD(byte panelsWide, byte panelsHigh, byte pin_noe, byte pin_a, byte pin_b, byte pin_sck,
          byte pin_clk, byte pin_r_data, byte pin_g_data)
  : BaseDMD(panelsWide, panelsHigh, pin_noe, pin_a, pin_b, pin_sck),
    pin_clk(pin_clk),
    pin_r_data(pin_r_data),
    pin_g_data(pin_g_data)
{
}
...
void SoftDMD::beginNoTimer()
{
  ...
  digitalWrite(pin_g_data, LOW);
  pinMode(pin_g_data, OUTPUT);

  BaseDMD::beginNoTimer();
}


static inline __attribute__((always_inline)) void softSPITransfer(uint8_t data, volatile port_reg_t *data_port, port_reg_t data_mask, volatile port_reg_t *data_port2, port_reg_t data_mask2, volatile port_reg_t *clk_port, port_reg_t clk_mask) {
...
void SoftDMD::writeSPIData(volatile uint8_t *rows[4], const int rowsize)
{
  ...
  volatile port_reg_t *port_green_data = (volatile port_reg_t *) portOutputRegister(digitalPinToPort(pin_g_data));
  port_reg_t mask_green_data = digitalPinToBitMask(pin_g_data);

  volatile port_reg_t *port_red_data = (volatile port_reg_t *) portOutputRegister(digitalPinToPort(pin_r_data));
  port_reg_t mask_red_data = digitalPinToBitMask(pin_r_data);

  for(int i = 0; i < rowsize; i++) {
    softSPITransfer(*(rows[3]++), port_red_data, mask_red_data, port_green_data, mask_green_data, port_clk, mask_clk);
    softSPITransfer(*(rows[2]++), port_red_data, mask_red_data, port_green_data, mask_green_data, port_clk, mask_clk);
    softSPITransfer(*(rows[1]++), port_red_data, mask_red_data, port_green_data, mask_green_data, port_clk, mask_clk);
    softSPITransfer(*(rows[0]++), port_red_data, mask_red_data, port_green_data, mask_green_data, port_clk, mask_clk);
  }
}
//#endif
